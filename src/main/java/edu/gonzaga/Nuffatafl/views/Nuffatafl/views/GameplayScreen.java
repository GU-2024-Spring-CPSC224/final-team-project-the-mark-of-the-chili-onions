...
import edu.gonzaga.Nuffatafl.views.Nuffatafl.backend.GameManager;
import edu.gonzaga.Nuffatafl.views.Nuffatafl.backend.Player;
import edu.gonzaga.Nuffatafl.views.Nuffatafl.backend.Team;
import edu.gonzaga.Nuffatafl.views.Nuffatafl.viewHelpers.Theme;
import edu.gonzaga.Nuffatafl.views.Nuffatafl.viewHelpers.ThemeComponent;
import edu.gonzaga.Nuffatafl.views.Nuffatafl.viewNavigation.Screen;
...
public class GameplayScreen extends JPanel {
    ...
    public GameplayScreen(StateController stateController) {
        ...
        stateController.gameManager.onAttackerPlayerChange(event -> {
    ...
    private void handleTeamSwitch() {
        Player attacker = stateController.gameManager.getAttackerPlayer();
        Player defender = stateController.gameManager.getDefenderPlayer();

        switch (stateController.gameManager.getCurrentTeam()) {
            case ATTACKER -> {
                attackerLabel.setBorder(new LineBorder(attacker.color, 2));
                defenderLabel.setBorder(new EmptyBorder(0, 0, 0, 0));
            }
            case DEFENDER -> {
                defenderLabel.setBorder(new LineBorder(attacker.color, 2));
